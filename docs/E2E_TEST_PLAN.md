...
| 프레임워크 | 용도 | 선정 이유 |
|-----------|------|----------|
| **Playwright + Electron** | E2E 테스트 | Electron 공식 지원, headless 모드, 크로스 플랫폼, 빠른 실행 |
| **Vitest** | 유닛/통합 테스트 | Vite 네이티브 통합, ESM 지원, Jest 호환 API |
| **@testing-library/react** | 컴포넌트 테스트 | DOM 기반 테스트, 접근성 중심 쿼리 |
